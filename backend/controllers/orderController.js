...
const user_frontend_url = process.env.USER_FRONTEND_URL || "http://localhost:5174";
const admin_frontend_url = process.env.ADMIN_FRONTEND_URL || "http://localhost:5173";

// Function to place an order
const placeOrder = async (req, res) => {
    try {
        // Determine the frontend URL based on the order type (user or admin)
        const frontend_url = req.body.isClientOrder ? admin_frontend_url : user_frontend_url;
        ...
        line_items.push({
            price_data: {
                ...
                unit_amount: 2 * 100 * 80, // Delivery charges in cents
            },
            quantity: 1,
        });
